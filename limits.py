...
from ROOT import TFile,gROOT,THStack,TCanvas,TLegend,kRed,kYellow
...
gROOT.SetBatch()
postfix=''
path_base='/scratch/hh/dust/naf/cms/user/usai/ZprimeFullHad/'
...
histo_folder_list=['NoCutsHistos','TriggerHistos','had_0btag_m','had_1btag_m','had_2btag_m','had_0btag_l','had_1btag_l','had_2btag_l','had_0btag_mn','had_1btag_mn','had_2btag_mn','had_0btag_ln','had_1btag_ln','had_2btag_ln','had_0btag_my','had_1btag_my','had_2btag_my','had_0btag_ly','had_1btag_ly','had_2btag_ly','had_0btag_mny','had_1btag_mny','had_2btag_mny','had_0btg_lny','had_1btag_lny','had_2btag_lny','antitag_den','antitag_num_l','antitag_num_m','antitag_num_ln','antitag_num_mn']
histo_name_list=["Ntopjets","SumOfTopCandidatesPt","LeadingTopCandidatePt","SubLeadingTopCandidatePt","TopCandidate1Pt","TopCandidate2Pt","Mtt","Nevts","ptNevts","toppt","DeltaY","Nsub"]
...
if doplots:
  ...
  def make_plot(histo_name,cut_name):
    ...
    cmslabel.SetNDC()
    signal1000=signal_files[process_names_signal_narrow.index('1000')]
    signal2000=signal_files[process_names_signal_narrow.index('2000')]
    signal3000=signal_files[process_names_signal_narrow.index('3000')]
    ...
    legend.AddEntry(signal1000,"Z'1TeV",'l').Get(cut_name+'/'+histo_name).Clone('z1000'+us+histo_name+us+cut_name)
    legend.AddEntry(signal2000,"Z'2TeV",'l').Get(cut_name+'/'+histo_name).Clone('z2000'+us+histo_name+us+cut_name)
    legend.AddEntry(signal3000,"Z'3TeV",'l').Get(cut_name+'/'+histo_name).Clone('z3000'+us+histo_name+us+cut_name)
    ...
    outfile.cd(cutname)
    canvas.Write()
    canvas.SaveAs(path_base+'pdf/'+canvas.GetName()+'.pdf')
...
if dolimits:
  ...
  def make_theta_rootfile(theta_root_filename,cut_index):
    limits_input_file=TFile(path_base+theta_root_filename+".root","RECREATE")
    #limits_input_file.cd()
    for category_index in range(len(categories)):
      ttbar_histo=ttbar_file.Get(base_histoname+us+categories[category_index]+us+cut_list[cut_index]+'/Mtt').Clone(base_histoname+us+categories[category_index]+us+us+'ttbar')
      ttbar_histo.Rebin(2)
      qcd_histo=qcd_file.Get(base_histoname+us+categories[category_index]+us+cut_list[cut_index]+'/Mtt').Clone(base_histoname+us+categories[category_index]+us+us+'qcd')
      qcd_histo.Rebin(2)
      ttbar_histo.Write()
      qcd_histo.Write()
      signal_histos=[]
      for i in range(len(process_list_signal_narrow)):
	tmp_histo =signal_files[i].Get(base_histoname+us+categories[category_index]+us+cut_list[cut_index]+'/Mtt').Clone(base_histoname+us+categories[category_index]+us+us+process_list_signal_narrow[i].split('.')[2].split('W')[0])
	tmp_histo.Rebin(2)
  ...
  catstring+='btag'
  theta_input_name='zphad'+us+catstring+us+cut_type
  make_theta_rootfile(theta_input_name,cut_list.index(cut_type))
...
report.write_html('"+path_base+"htmlout"+us+theta_input_name+"')")
  ...
  system('python /afs/naf.desy.de/user/u/usai/scratch/theta/utils2/theta-auto.py '+path_base+theta_input_name+'.py')
